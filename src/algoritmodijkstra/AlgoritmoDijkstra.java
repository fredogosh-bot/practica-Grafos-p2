...
public class AlgoritmoDijkstra {
   ...
   // --- ALGORITMO DE DIJKSTRA ---
    public void dijkstra(int nodoOrigen) {
        ...
        int[] D = new int[numVerts];
        boolean[] F = new boolean[numVerts];
        ...
        for (int i = 0; i < numVerts; i++) {
            ...
            int u = buscarDistanciaMinima(distancias, F);
            ...
            for (int v = 0; v < numVerts; v++) {
                ...
                if (adyacente(u,v) && !F[v]) {
                    ...
                    if (nuevoCosto < distancias[v]) {
                        ...
                        D[v] = u; // Guardamos que llegamos a 'v' desde 'u'
                    }
                }
            }
        }
        ...
        imprimirSolucionDijkstra(nodoOrigen, distancias, D);
    }

    // Método auxiliar para encontrar el nodo más cercano no visitado
    private int buscarDistanciaMinima(int[] distancias, boolean[] F) {
        int min = Integer.MAX_VALUE;
        int indiceMin = -1;

        for (int i = 0; i < numVerts; i++) {
            if (!F[i] && distancias[i] <= min) {
                min = distancias[i];
                indiceMin = i;
            }
        }
    ...
    private void imprimirSolucionDijkstra(int origen, int[] distancias, int[] d) {
    ...
    private void imprimirCamino(int actual, int[] D) {
        if (actual == -1) return;
        
        imprimirCamino(D[actual], D); // se implementa recursividad
        System.out.print("v" + (actual + 1) + (D[actual] != -1 ? "-" : "")); // Imprime al volver
